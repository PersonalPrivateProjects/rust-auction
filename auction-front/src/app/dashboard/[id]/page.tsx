...
import { getSubastaById, getPujas, iniciarSubasta, finalizarSubasta } from "../../subastasProxy";
...
export default function SubastaDetallePage() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      const found = await getSubastaById(wallet, id);
  ...
  async function handleIniciar() {
    ...
    const refreshed = await getSubastaById(wallet, id);
    setSubasta(refreshed);
  }
  ...
  if (!id) return <div>No se proporcionó ID de subasta.</div>;
  if (loading) return <div>Cargando...</div>;
  if (!subasta) return <div>Subasta no encontrada.</div>;

  const isOwner = walletAddress && walletAddress === subasta.creador;
  ...
  return (
    <div style={{ maxWidth: 1024, margin: "2rem auto", padding: 16, fontFamily: "Inter, sans-serif" }}>
      ...
      >
        <div style={{ fontSize: 48, background: "#fff3", borderRadius: "50%", padding: 18, display: "flex" }}>
          <FaGavel />
        </div>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 4 }}>
            ...
            <span style={{ fontSize: 15 }}>
              <b>Inicio:</b> {new Date(Number(subasta.fecha_inicio) * 1000).toLocaleString()}
            </span>
            <span style={{ fontSize: 15, color: "#e0e7ef" }}>|</span>
            <span style={{ fontSize: 15 }}>
              <b>Fin:</b> {new Date(Number(subasta.fecha_fin) * 1000).toLocaleString()}
            </span>
          </div>

          {isOwner && subasta.estado === "creada" && (
            <button
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ fontFamily: "monospace", fontSize: 14 }}>{subasta.publicKey}</span>
        </div>

        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <FaUser style={{ color: "#2563eb", fontSize: 20 }} />
          <span style={{ fontWeight: 700 }}>Creador:</span>
          <span style={{ background: "#e0e7ef", color: "#2563eb", borderRadius: 8, padding: "2px 8px", fontFamily: "monospace", fontSize: 14 }}>
            {subasta.creador}
          </span>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ background: "#bbf7d0", color: "#166534", borderRadius: 8, padding: "2px 8px", fontFamily: "monospace", fontSize: 14 }}>
            {subasta.ganador}
          </span>
        </div>
      ...
      {pujas.length === 0 ? (
        <div style={{ background: "#fef3c7", color: "#92400e", padding: 18, borderRadius: 10, marginBottom: 28, fontWeight: 600 }}>
          No hay pujas para esta subasta.
        </div>
      ) : (
        <div style={{ overflowX: "auto", marginBottom: 28 }}>
          <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0, background: "#f8fafc", borderRadius: 14, overflow: "hidden" }}>
            <thead>
              <tr style={{ background: "#3c97ff" }}>
                <th style={{ padding: 12, fontWeight: 800, fontSize: 16 }}>#</th>
                <th style={{ padding: 12, fontWeight: 800, fontSize: 16 }}>Importe</th>
                <th style={{ padding: 12, fontWeight: 800, fontSize: 16 }}>Usuario</th>
                <th style={{ padding: 12, fontWeight: 800, fontSize: 16 }}>Fecha</th>
                <th style={{ padding: 12, fontWeight: 800, fontSize: 16 }}>PDA</th>
              </tr>
            </thead>
            <tbody>
              {pujas
                ...
                .map((p, idx) => (
                  <tr key={p.publicKey} style={{ color: "#111", background: idx % 2 === 0 ? "#f1f5f9" : "#fff" }}>
                    ...
                    <td style={{ padding: 12, fontFamily: "monospace", fontSize: 15 }}>{p.pk}</td>
                    <td style={{ padding: 12 }}>{new Date(Number(p.ts)).toLocaleString()}</td>
                    <td style={{ padding: 12, fontFamily: "monospace", fontSize: 13, color: "#64748b" }}>
